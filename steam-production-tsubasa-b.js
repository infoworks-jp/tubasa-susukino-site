(()=>{
'use strict';
const canvas=document.querySelector('.signature-tsubasa .steam-tsubasa-b');
if(!canvas)return;
...
void main(){
 ...
 float sy=mix(.32,.42,mobileBias);
 vec2 srcs[4]; srcs[0]=vec2(.570,sy+.010);srcs[1]=vec2(.625,sy-.006);srcs[2]=vec2(.680,sy+.008);srcs[3]=vec2(.735,sy-.004);
 for(int i=0;i<4;i++){
 ...
 if(pointerActive>.5){
   ...
   float rad=0.090 + min(length(pointerVel)*.75,.12);
   float g=exp(-dot(q,q)/(rad*rad));
   vec2 pv=pointerVel; pv.x/=max(aspect,.001); pv.y*=1.18;
   v += pv*g*0.82;
   d += g*(0.050 + min(length(pointerVel)*0.10,0.030));
 }
