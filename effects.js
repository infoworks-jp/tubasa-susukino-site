...
uniform float time,seed,motionAmount;
...
void main(){
 ...
 float veil=(1.-exp(-d*2.8))*(.2+.8*edge)*(.35+.65*detail)*mask*.10*motionAmount;
 ...
 o=vec4(color,1.);`,
  ...
  function step(s, dt, t, dyeDissipation = 0.9) {
    ...
    u = use("advection");
    gl.uniform1f(u.dt, dt);
    gl.uniform1f(u.dissipation, 0.7);
    bind(u, "velocity", s.velocity.read.texture, 0);
  ...
  function render(s, t) {
    ...
    gl.uniform1f(u.motionAmount, motion.matches ? 0 : 1);
    draw(null);
