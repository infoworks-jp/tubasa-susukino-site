(()=>{
'use strict';
const canvas=document.querySelector('.signature-tsubasa .steam-tsubasa-a');
if(!canvas)return;
const photoAnchors=[[470,480],[950,430]];
const visualStyle=document.createElement('style');visualStyle.textContent='.signature-tsubasa .fx-canvas[data-effect="steam"]{opacity:.68!important}';document.head.appendChild(visualStyle);
...
uniform vec2 pointer;uniform vec2 pointerVel;uniform float pointerActive;uniform float hold;uniform float autoPulse;uniform vec2 sourceLeft;uniform vec2 sourceRight;
...
void main(){
 ...
 // The two photographed plumes use two Lab emitters each.  No synthetic
 // center plume is added between the ramen and ikura bowls.
 vec2 srcs[4]; srcs[0]=sourceLeft+vec2(-.028,.010);srcs[1]=sourceLeft+vec2(.028,-.006);srcs[2]=sourceRight+vec2(-.035,.008);srcs[3]=sourceRight+vec2(.035,-.004);
 for(int i=0;i<4;i++){
